...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BY_USER, query = "SELECT m FROM Meal m WHERE m.user.id=:user_id ORDER BY m.dateTime DESC"),
        @NamedQuery(name = Meal.BY_ID, query = "SELECT m FROM Meal m WHERE  m.id=:id AND m.user.id=:user_id"),
        @NamedQuery(name = Meal.BETWEEN_DATES, query = "SELECT m FROM Meal m WHERE m.user.id=:user_id AND m.dateTime BETWEEN :start AND :end ORDER BY m.dateTime DESC "),
})
@Entity
